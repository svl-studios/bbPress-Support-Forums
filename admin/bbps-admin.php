...
function bbps_extend_forum_attributes_mb( $forum_id ) {
	...
	<p>
		<strong><?php esc_html_e( 'Support Forum:', 'bbps' ); ?></strong>
		<input type="checkbox" name="bbps-support-forum" value="1" <?php echo esc_attr( $checked1 ); ?>/>
		<br/>
		<!-- <small>Click here To learn more about the support forum setting.</small> -->
	</p>
	<p>
		<strong><?php esc_html_e( 'Voting Forum:', 'bbps' ); ?></strong>
		<input type="checkbox" name="bbps-voting-forum" value="1" <?php echo esc_attr( $checked2 ); ?>/>
		<br/>
		<!-- <small>Click here To learn more about the support forum setting.</small> -->
	</p>
...
function bbps_forum_attributes_mb_save( $forum_id ) {
	...
	if ( wp_verify_nonce( sanitize_key( wp_unslash( $_POST['_wpnonce'] ?? '' ) ), $nonce_action ) ) {
		...
		if ( ! empty( $_POST['bbps-premium-forum'] ) ) {
			update_post_meta( $forum_id, '_bbps_is_premium', sanitize_key( wp_unslash( $_POST['bbps-premium-forum'] ) ) );
		}
		...
		if ( ! empty( $_POST['bbps-support-forum'] ) ) {
			update_post_meta( $forum_id, '_bbps_is_support', sanitize_key( wp_unslash( $_POST['bbps-support-forum'] ) ) );
		}
		...
		if ( ! empty( $_POST['bbps-voting-forum'] ) ) {
			update_post_meta( $forum_id, '_bbps_is_voting', sanitize_key( wp_unslash( $_POST['bbps-voting-forum'] ) ) );
		}
 ...
 *
 * @param array $input Unput values.
 *
 * @return array
 */
function bbps_validate_checkbox_group( ?array $input ): ?array {
 ...
 * @return mixed
 */
function bbps_validate_options( array $input ) {
